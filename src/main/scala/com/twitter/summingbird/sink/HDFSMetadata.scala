...
import com.twitter.bijection.json.{ JsonBijection, JsonObject }
...
object HDFSMetadata {
  ...
  def getPath(conf: Configuration, rootPath: String): Option[Path] = {
    ...
    val vs = new VersionedStore(fs, rootPath)
    Option(vs.mostRecentVersionPath) map { new Path(_, METADATA_FILE) }
  }

  def get[T: JsonObject](conf: Configuration, path: String): Option[T] = {
    getPath(conf, path)
      .map { p =>
        val fs = FileSystem.get(conf)
  ...
  def put[T: JsonObject](conf: Configuration, path: String, obj: Option[T]) {
    val meta = obj
      .map { JsonBijection.toString[T](_) }
      .getOrElse("")
