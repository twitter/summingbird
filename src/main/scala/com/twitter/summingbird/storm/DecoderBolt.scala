...
// RPC call.
class DecoderBolt[Key](@transient rpc: Bijection[Key,String]) extends BaseBasicBolt {
  import Constants._

  val rpcBijection = MeatLocker(rpc)

  override def execute(tuple: Tuple, collector: BasicOutputCollector) {
    val key = rpcBijection.get.invert(tuple.getString(0))
    val returnInfo = tuple.getString(1)

    collector.emit(new Values(key.asInstanceOf[AnyRef], returnInfo))
  }

  override def declareOutputFields(declarer : OutputFieldsDeclarer) {
    declarer.declare(new Fields(AGG_KEY, RETURN_INFO))
  }
}
