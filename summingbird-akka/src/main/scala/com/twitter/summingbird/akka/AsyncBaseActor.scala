...
abstract class AsyncBaseActor[I, O](
  maxWaitingFutures: MaxWaitingFutures,
  hasDependants: Boolean,
  targetNames: List[String]) extends BaseActor[I, O](hasDependants, targetNames) {
   ...
   * doing a store merge
   */
  def apply(tup: WireType[I], in: (Timestamp, I)): Future[Iterable[(JList[WireType[I]], Future[TraversableOnce[(Timestamp, O)]])]]

  private lazy val outstandingFutures = Queue[Future[Unit]]()
  private lazy val responses = Queue[(JList[WireType[I]], Try[TraversableOnce[(Timestamp, O)]])]()

  def receive = {
      case w@WireType[_] => execute(w.asInstanceOf[WireType[I]])
  }

  def execute(tuple: WireType[I]) {
    val fut = apply(tuple, tuple.toFMInput)
      .onSuccess { iter =>
