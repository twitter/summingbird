  ...
  def fromTraversable[T](items: TraversableOnce[T]): AkkaSource[T] =
    new AkkaSource[T] {
      ...
      override def poll = {
        ...
        Future.value(items.map((startTs, _)))
      }
    }

  def fromFn[T](fn: () => TraversableOnce[(Timestamp, T)]): AkkaSource[T] =
    new AkkaSource[T] {
      private var finished = false
      override def isFinished = finished
...
trait AkkaSource[+T] extends Serializable { self =>
   ...
   */
  def poll: Future[TraversableOnce[(Timestamp, T)]]
