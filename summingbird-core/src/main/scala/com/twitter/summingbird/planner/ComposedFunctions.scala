 ...
 */
case class KeyFlatMapFunction[K1, K2, V](fn: K1 => TraversableOnce[K2]) extends (Tuple2[K1, V] => TraversableOnce[(K2, V)]) with FunctionContainer {
  def apply(kv: (K1, V)) = fn(kv._1).map((_, kv._2))
  def innerFunctions = FunctionContainer.flatten(fn)
}

/**
 * flatMapping values can be done by a normal flatmap
 */
case class ValueFlatMapFunction[K, V1, V2](fn: V1 => TraversableOnce[V2]) extends (Tuple2[K, V1] => TraversableOnce[(K, V2)]) with FunctionContainer {
