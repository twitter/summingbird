...
object AlexPlatformLaws extends Specification {
  ...
  def testGraph[T: Manifest: Arbitrary, K: Arbitrary, V: Monoid: Arbitrary: Equiv] =
    new TestGraphs[AlexPlatform, T, K, V](new AlexPlatform)(
      () => MutableMap.empty[K, V])(() => new BufferFunc[T])(
      AlexPlatform.toSource(_))(s => { s.get(_) })({ (f, items) =>
        f.asInstanceOf[BufferFunc[T]].buf.toList == items
      })({ (p: AlexPlatform, plan: AlexPlatform#Plan[_]) => p.run(plan) })
  ...
  def mapKeysChecker[T: Manifest: Arbitrary, K1: Arbitrary, K2: Arbitrary,
    ...
    val platform = new AlexPlatform
    val currentStore: AlexPlatform#Store[K2, V] = MutableMap.empty[K2, V]
    val sourceMaker = AlexPlatform.toSource[T](_)
    ...
    // supplied store.
     val plan = platform.plan {
       TestGraphs.singleStepMapKeysJob[AlexPlatform, T, K1, K2, V](sourceMaker(original), currentStore)(fnA, fnB)
     }
     platform.run(plan)
  ...
  def lookupCollectChecker[T:Arbitrary:Equiv:Manifest, U:Arbitrary:Equiv]: Boolean = {
    val mem = new AlexPlatform
    val input = sample[List[T]]
    val srv = sample[T => Option[U]]
    var buffer = Vector[(T,U)]() // closure to mutate this
    val prod = TestGraphs.lookupJob[AlexPlatform,T,U](AlexPlatform.toSource(input), srv, { tu: (T,U) => buffer = buffer :+ tu })
  ...
  "The AlexPlatform Platform" should {
    //Set up the job:
